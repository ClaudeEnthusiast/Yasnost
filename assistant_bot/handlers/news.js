...
const { collectNews, formatDigest, normTitle } = require('../services/news');
...
async function sendNewsDigest(bot, chatId, label) {
  ...
  const { items, errors } = await collectNews({ windowMs: 26 * 3600 * 1000, seen });
  ...
  await saveSeen(seenList.concat(items.map((it) => normTitle(it.title))));
  return true;
}
  ...
  bot.onText(/^\/(новости|news)\b/i, async (msg) => {
    ...
    try {
      ...
      if (!sent) {
        ...
        const { items } = await collectNews({ windowMs: 26 * 3600 * 1000 });
